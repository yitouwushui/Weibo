...
import com.yitouwushui.weibo.view.pulltorefreshview.internal.HBLoadingLayout;
import com.yitouwushui.weibo.view.pulltorefreshview.internal.LoadingLayout;
import com.yitouwushui.weibo.view.pulltorefreshview.internal.Utils;
import com.yitouwushui.weibo.view.pulltorefreshview.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    protected boolean mCanRefresh = true;// 是否可以刷新，同步设置是否到底自动加载
    ...
    private ImageView mRefreshCatImageView;// 刷新的小猫图片
    ...
    public final void setCurrentMode(Mode mCurrentMode) {
    ...
    public void onRefreshComplete() {
        if (isRefreshing()) {
            if (mOnRefreshStatuListener != null) {
                mOnRefreshStatuListener.finish();
            }
            if (mOnDownPxListener != null) {
                // 如果是上拉，里面是不会执行的
                mOnDownPxListener.finish();
            }
            if (mOnDownPxListener != null && mFooterLayout.getVisibility() == VISIBLE) {
                // 上拉的完成了
...
//                    if (!isRefreshing())
            ...
            case MotionEvent.ACTION_UP: {
                // 滑动松开
                if (mOnDownPxListener != null)
                    mOnDownPxListener.up();
                if (mIsBeingDragged) {
                    mIsBeingDragged = false;

                    if (mState == State.RELEASE_TO_REFRESH
                            && (null != mOnRefreshListener || null != mOnRefreshListener2)) {
                        // 延迟属性动画的时间，避免动画还没完成，加载就完成了，导致设置图片的时候卡顿
...
//            mHeaderLayout.refreshing(mShowHeadView);
...
//        // 限制底部的上滑距离
//        if (value > 0) {
//            value = Math.max(value, getFooterSize());
//        }
        ...
        switch (getPullToRefreshScrollDirection()) {
            case VERTICAL:
                if (mShowHeadView || value > 0)
                    scrollTo(0, value);
                if (value > 0 && mOnUpPxListener != null) {
                    mOnUpPxListener.onUpPx(value);
                }
                break;
            case HORIZONTAL:
                scrollTo(value, 0);
    ...
    protected void callRefreshListener() {
        ...
        if (null != mOnRefreshListener) {
            if (mCurrentMode == Mode.PULL_FROM_START) {
                ...
                if (mOnRefreshStatuListener != null) {
                    mOnRefreshStatuListener.onRefresh(this);
                }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrShowHeadView)) {
            mShowHeadView = a.getBoolean(
                    R.styleable.PullToRefresh_ptrShowHeadView, true);
        }
        if (a.hasValue(R.styleable.PullToRefresh_ptrAutoLoad)) {
            mAutoLoad = a.getBoolean(
                    R.styleable.PullToRefresh_ptrAutoLoad, true);
            if (mMode != Mode.BOTH || mMode != Mode.PULL_FROM_END) {
                mAutoLoad = false;
            }
        }

        if (a.hasValue(R.styleable.PullToRefresh_ptrCanRefresh)) {
            mCanRefresh = a.getBoolean(
                    R.styleable.PullToRefresh_ptrCanRefresh, true);
            if (!mCanRefresh) {
                mAutoLoad = false;
                mShowHeadView = false;
            }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            ...
            default:
                ...
                if (mOnDownPxListener != null || mOnUpPxListener != null) {
                    int maximumPullScroll = getMaximumPullScroll();
                    int value = Math.min(maximumPullScroll, Math.max(-maximumPullScroll, newScrollValue));
                    if (value < 0) {
                        if (mOnDownPxListener != null)
                            mOnDownPxListener.onDownPx(mLastMotionY - mInitialMotionY);
                    } else if (value > 0) {
                        if (mOnUpPxListener != null)
                            mOnUpPxListener.onUpPx(mLastMotionY - mInitialMotionY);
                    }
                }
                break;
        }
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            return new HBLoadingLayout(context, mode, scrollDirection, attrs);
        }
    }
    ...
    public static enum ScrollViewStyle {
         ...
         * int.
         *
         * @param styleInt - int to map a scrollStyle to
         * @return ScrollViewStyle that styleInt maps to, or NORMAL by default.
         */
        static ScrollViewStyle mapIntToValue(final int styleInt) {
            for (ScrollViewStyle value : ScrollViewStyle.values()) {
                if (styleInt == value.getIntValue()) {
                    return value;
                }
            }
        ...
        static ScrollViewStyle getDefault() {
            return NORMAL;
        }

        private int mIntValue;

        // The scrollStyeInt values need to match those from attrs.xml
        ScrollViewStyle(int scrollStyeInt) {
            mIntValue = scrollStyeInt;
        }
    ...
    public static interface OnRefreshListener<V extends View> {
        ...
        public void onLoadMore(final PullToRefreshBase<V> refreshView);
    ...
    private void setmOnDownPxListener(OnDownPxListener mOnDownPxListener) {
        this.mOnDownPxListener = mOnDownPxListener;
    }

    private interface OnDownPxListener<T extends View> {
        void onDownPx(float value);
    ...
    // 上移距离的监听
    public void setmOnUpPxListener(OnUpPxListener mOnUpPxListener) {
        this.mOnUpPxListener = mOnUpPxListener;
    }

    public interface OnUpPxListener {
        void onUpPx(float value);
    }

    public void setmOnRefreshStatuListener(OnRefreshStatuListener mOnRefreshStatuListener) {
        this.mOnRefreshStatuListener = mOnRefreshStatuListener;
    }
    ...
    public interface OnRefreshStatuListener<T extends View> {
        ...
        // 通知开始刷新了，变换图片效果
        void onRefresh(PullToRefreshBase<T> tPullToRefreshBase);

    }

    public void setmCanRefresh(boolean mCanRefresh) {
        this.mCanRefresh = mCanRefresh;
    }

    private boolean isAutoLoadNow = false;
    ...
    public void setShowHeadView(boolean showHeadView) {
        mShowHeadView = showHeadView;
    }

    public boolean getShowHeadView() {
        return mShowHeadView;
    }
